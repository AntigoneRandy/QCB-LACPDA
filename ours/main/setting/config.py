...
def pq_cifar_fp():
    ...
    from model.Resnet_new_PQ_old import resnet_quantized
...
def pq_tiny_fp():
    from dataset.datasets import Tiny as Tiny_stage2
    ...
    return model,train_loader,val_loader,trainloader_bd,valloader_bd,val_loader_no_targets
def ca_tiny_fp_8():
    from dataset.tiny_1stage import Tiny
...
def pq_cifar_fp_new():
    ...
    val_loader_no_targets = data.get_asrnotarget_loader()
    from model.resnet import ResNet18
    path = "/root/autodl-tmp/Boheng/quantization-backdoor/pt/results/resnet_cifar10_rm/ckpt.pth"
    model = ResNet18(num_classes=10,dataset='cifar10') 
    checkpoint = torch.load(path,map_location='cuda')
    model.load_state_dict(checkpoint['net'],strict=False)
