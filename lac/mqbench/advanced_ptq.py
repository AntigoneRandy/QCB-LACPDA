...
class LossFunction:
    ...
    def __call__(self, pred, tgt):
        ...
        total_loss = rec_loss
        if self.count % 500 == 0:
            logger.info('Total loss:\t{:.3f} \tcount={}'.format(
...
def subgraph_reconstruction_dc(subgraph, cached_inps, cached_oups, config):
    global USE_LINK
    global USE_DDP
    device = next(subgraph.parameters()).device
    w_para, a_para = [], []
    ...
    for i in range(config.max_count):
        ...
        out_final = save_dc_fp_data(subgraph, *cur_args)
        err = loss_func(out_quant, out_final)
        err /= world_size
        err.backward()
        if world_size > 1:
            for param in w_para:
...
def extract_fp_subgraph(orig_module: nn.Module, nodes: List[fx.Node], output: fx.Node, g2node: dict, qnode2fpnode_dict: dict):
    """
    ...
    # Create input nodes for nodes that are not part of the subgraph
    for node in nodes:
    ...
    # Copy nodes and create mapping in the new graph
    for node in nodes:
        if node in inp_lst:
            continue
        if node in g2node:
        ...
        # Use qnode2fpnode_dict to get the corresponding full precision node
        fp_node = qnode2fpnode_dict.get(node)
        if fp_node is None:
            raise ValueError(f"No corresponding full precision node found for {node}")

        new_node = new_graph.node_copy(fp_node, lambda x: env[x])
        env[node] = new_node
...
def ptq_reconstruction_pda(model: GraphModule, cali_data: list, config: dict, graph_module_list: list = None):
    r"""
    ...
    for node in nodes:
        ...
        if node.op == "call_module" and isinstance(quant_modules[node], _ADAROUND_SUPPORT_TYPE):
            ...
            for _node in layer_node_list:
                if all([arg in layer_node_list for arg in _flatten_args(_node.args) if isinstance(arg, torch.fx.Node)]):
                    ...
                    _, fp32_inps = save_inp_oup_data(fp32_model, None, fp32_inp_module, cali_data, 
                                                     store_inp=False, store_oup=0.5, keep_gpu=config.keep_gpu)
            ...
            # disable_all(subgraph)
            # print(fp32_all_inps)
            subgraph = subgraph.to('cuda')
            cached_recitfy = save_dc_fp_data(model = quant_model,layer = subgraph, cali_data = cached_inps,config = config)
            cached_inps = []
            cached_inps.append(cached_recitfy)
...
def save_dc_fp_data(model, layer, config,cali_data: torch.Tensor,
                    batch_size: int = 32, keep_gpu: bool = True,
                    input_prob: bool = False, lamb=50, bn_lr=4e-6):
    """Activation after correction"""
    device = next(model.parameters()).device
    get_inp_out = GetDcFpLayerInpOut(model, layer, device=device, input_prob=input_prob, lamb=lamb, bn_lr=bn_lr,config=config)
    cached_batches = []
    cali_data = cali_data[0]
    print("Start correcting {} batches of data!".format(int(len(cali_data))))
    for i in trange(int(len(cali_data))):
            cur_out = get_inp_out(cali_data[i])
...
class GetDcFpLayerInpOut:
    def __init__(self, model, layer,config,
    ...
    def __call__(self, model_input):
        ...
        if len(hooks) == 0:
            logger.info('no bn layer in this layer')
            return model_input
        input_sym = model_input.clone()
        para_input = input_sym.data.clone()
        para_input = para_input.to(self.device)
       
        iters = self.config.pda_count
        scheduler = [250, 500, 1000, 2000, 3000, 4000]
        alpha=self.config.pda_alpha
        eps=self.config.pda_eps
        for iter in range(iters):
            ...
            if iter in scheduler:
                alpha /= 2

            
            adv_input = para_input + alpha*para_input.grad.sign()
            eta = torch.clamp(adv_input - input_sym, min=-eps, max=eps)
            para_input = (input_sym + eta)
            para_input = para_input.detach_()

        return para_input.detach()
