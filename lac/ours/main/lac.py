...
from mqbench.advanced_ptq import ptq_reconstruction,ptq_reconstruction_pda
...
@hydra.main(config_path="../config", config_name="r18_4_4.yaml")
def test(config):
    ...
    if not hasattr(config, 'quantize'):
        ...
        dataset_act = val_loader.dataset
        val_loader_act = torch.utils.data.DataLoader(dataset_act, batch_size=32, shuffle=True, num_workers=16, pin_memory=True)
        cali_data_act = load_calibrate_data(val_loader_act, cali_batchsize=config.quantize.cali_batchsize)
        ...
        with torch.no_grad():
            ...
            for batch in cali_data_act:
                model(batch.cuda())
            enable_calibration_woquantization(model, quantizer_type='weight_fake_quant')
            model(cali_data_act[0].cuda())
        ...
        if hasattr(config.quantize, 'reconstruction'):
            if config.pda == True:
                print("use pda")
                model = ptq_reconstruction_pda(
                    model, cali_data, config.quantize.reconstruction)
            else:
                print("do not use pda")
        ...
        print("cda")
        cda = evaluate(val_loader, model)
        print("asr")
        evaluate(valloader_bd, model)
        print("asr_no_targets")
        asr = evaluate(val_loader_no_targets, model)
